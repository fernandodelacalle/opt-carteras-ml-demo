...
class CriticalLineAlgorithm:
    ...
    def __init__(self, weight_bounds=(0, 1), calculate_expected_returns="mean"):
        ...
        # TDR: Set this att to false to reproduce original behaviour
        # CLA search get stagnated in some scenarios.  
        # See _check_free_cycle below
        self.check_free_repeated = False
    ...
    def allocate(self,
        ...
        while True:
            ...
            if ithr > 300:
                logger.warning("Looping out of CLA, limit iteration reached")
                break
    ...
    def _check_free_cycle(self, current_free, new_var, out_var=True):
        """TDR: The step of making bound a free variable may incurr in a quasi-infinte
        loop becasue current free-vars solution is equivalent to the new bound
        In this function we check if the set of free variables has been 
        already visited
        """
        if not self.check_free_repeated:
            return False

        check_free = current_free.copy()
        if out_var:
            check_free.remove(new_var)
        else:
            check_free.append(new_var)

        fw_sorted = [fw.copy() for fw in self.free_weights]

        if check_free in fw_sorted:
            return True
        else:
            return False
        ...
        if len(free_weights) > 1:
            ...
            for i in free_weights:
                ...
                if (self._infnone(lambda_i) > self._infnone(lambda_in)
                        and not self._check_free_cycle(free_weights, i)): 
    ...
    def _free_bound_weight(self, free_weights):
        ...
        if len(free_weights) < self.expected_returns.shape[0]:
            ...
            for i in bounded_weight_indices:
                ...
                is_cycle = self._check_free_cycle(free_weights, 
                                                  i, 
                                                  out_var=False)
                if (self.lambdas[-1] is None 
                    or (lambda_i < self.lambdas[-1] 
